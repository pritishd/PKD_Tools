...
import libWeights
import libUtilities
for module in libFile, libWeights, libUtilities:
    reload(module)
...
class ObjExportGUI(GUI):
    ...
    def _setup_(self):
        ...
        self.txtField = cmds.textFieldButtonGrp(
            text='Get Obj Exporter', buttonLabel='Folder')
        cmds.rowLayout(w=self.width, numberOfColumns=2)
    ...
    def _connect_commands_(self):
        cmds.textFieldButtonGrp(
            self.txtField, e=1, buttonCommand=lambda*args: self._get_folder_())
        cmds.button(
            self.exportButton, e=1, command=lambda*args: self._export_objs_())
    ...
    def _export_objs_(self):
        ...
        for geo in selection:
            ...
            libUtilities.melEval(
                'file -force -options "groups=0;ptgroups=0;materials=0;smoothing=0;normals=0" -typ "OBJexport" -pr -es "%s";' % path)

    def _get_folder_(self):
        res = cmds.fileDialog2(
            dialogStyle=1,
            fm=3,
            cap="Define Obj Export Folder",
            okc="Set",
            startingDirectory=pm.workspace.path)
        if res:
            cmds.textFieldButtonGrp(self.txtField, e=1, text=res[0])
