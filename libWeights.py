...
@brief This package allows you to import/export weights of various deformers. @details For a seamless import/export the following is assumed
<ol>
...
import libUtilities,libFile,libXml
reload(libUtilities)
reload(libXml)
...
class Weights(object):
  ...
  def export_weights(self):
    ...
    evalStatment = 'deformerWeights -export -method "index" -deformer "%s" -path "%s" "%s"'%(self.target_deformer,self.folder,self.file)
  ...
  def import_weights(self):
    ...
    evalStatment = 'deformerWeights -import -method "index" -deformer "%s" -path "%s" "%s"'%(self.target_deformer,self.folder,self.file)
...
class SkinWeights(Weights):
  ...
  def copy_weights(self,newTarget):
    ...
    pm.copySkinWeights(ss=self.target_deformer,ds=res,noMirror=True,surfaceAssociation="rayCast" ,influenceAssociation="closestJoint")
  ...
  def _create_deformers_(self):
    ...
    if not self.target_deformer:
      jnts = []
      noJnts = []
      #Alias for the joint list that from the import data. Make sure the data is list
...
class MultiWeights(Weights):
  ...
  def _export_individual_weights_(self):
    ...
    for self.target_deformer in self.target_deformers:
      ...
      evalStatment = 'deformerWeights -export -method "index" -deformer "%s" -path "%s" "%s"'%(self.target_deformer,self.target_folder,self.file)
      libUtilities.melEval(evalStatment)
  ...
  def _import_individual_weights_(self):
    ...
    for self.target_deformer in libXml.list_persist(self.import_data["Order"]):
      ...
      if weightInfo['deformerWeight'].has_key("weights"):
        evalStatment = 'deformerWeights -import -method "index" -deformer "%s" -path "%s" "%s"'%(self.target_deformer,self.target_folder,self.file)
        libUtilities.melEval(evalStatment)
...
class ClusterWeights(MultiWeights):
  ...
  def _get_deformer_data_(self):
    ...
    for self.target_deformer in self.target_deformers:
      ...
      info["Vertices"] = libUtilities.decompress_vertice_group(verticesSet.members())
  ...
  def _export_individual_weights_(self):
    ...
    for self.target_deformer in self.target_deformers:
      ...
      for index,niceName in zip(self.target_deformer.weightIndexList(),self.target_deformer.getTarget()):
        #Get the weights
        weight_cmd = '%s.inputTarget[0].inputTargetGroup[%i].targetWeights[0:%d]' % (self.target_deformer,index,vertices - 1)
      ...
      if libFile.exists(self.weight_file):
        ...
        for index,niceName in zip(self.target_deformer.weightIndexList(),self.target_deformer.getTarget()):
          ...
          if weightMap.has_key(niceName):
            ...
            #Set the weight
            weight_cmd = '%s.inputTarget[0].inputTargetGroup[%i].targetWeights[0:%d]' % (self.target_deformer,index,len(weights) - 1)
...
class WeightManager(object):
  ...
  def export_all(self):
    ...
    sel = libUtilities.get_selected(stringMode = True,commandMode=self.command_mode)
    if not sel:
